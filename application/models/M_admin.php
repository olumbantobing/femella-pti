...
class M_admin extends CI_Model
{
  ...
  public function tabelmasuk()
  {
    ...
    // $query = $this->db->select('masuk_gudang.no, masuk_gudang.id, stok_gudang.nama_barang, masuk_gudang.jumlah, masuk_gudang.tanggal, masuk_gudang.keterangan')
    //   ->from('masuk_gudang')->innerjoin('stok_gudang')
    //   ->on('masuk_gudang.id' == 'stok_gudang.id')
    //   ->get();
    return $query->result();
  }
  ...
  {
    $query = $this->db->query("SELECT MAX(id) as id from stok_gudang");
    $hasil = $query->row();
    return $hasil->id;
  }
}
